...
def write_results(prediction, confidence, num_clusses, nms_conf=0.4):
    ...
    for i in range(batch_size):
        ...
        max_conf, max_conf_score = torch.max(image_pred[:, 5:5 + num_clusses], 1)
...
def predict_transform(pred, inp_dim, anchors, num_classes, CUDA=True):
    ...
    bbox_atr = num_classes + 5
    num_anchors = len(anchors)

    pred = pred.view(batch_size, bbox_atr * num_anchors, grid_size * grid_size)
    pred = pred.transpose(1, 2).contiguous()
    pred = pred.view(batch_size, grid_size * grid_size * num_anchors, bbox_atr)
